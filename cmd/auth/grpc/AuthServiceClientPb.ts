/**
 * @fileoverview gRPC-Web generated client stub for grpc
 * @enhanceable
 * @public
 */

// Code generated by protoc-gen-grpc-web. DO NOT EDIT.
// versions:
// 	protoc-gen-grpc-web v1.4.2
// 	protoc              v3.6.1
// source: github.com/elojah/game_03/cmd/auth/grpc/auth.proto


/* eslint-disable */
// @ts-nocheck


import * as grpcWeb from 'grpc-web';

import * as github_com_elojah_game_03_pkg_user_dto_user_pb from '../../../../../../github.com/elojah/game_03/pkg/user/dto/user_pb';
import * as github_com_elojah_game_03_pkg_pbtypes_empty_pb from '../../../../../../github.com/elojah/game_03/pkg/pbtypes/empty_pb';
import * as github_com_elojah_game_03_pkg_pbtypes_string_pb from '../../../../../../github.com/elojah/game_03/pkg/pbtypes/string_pb';


export class AuthClient {
  client_: grpcWeb.AbstractClientBase;
  hostname_: string;
  credentials_: null | { [index: string]: string; };
  options_: null | { [index: string]: any; };

  constructor (hostname: string,
               credentials?: null | { [index: string]: string; },
               options?: null | { [index: string]: any; }) {
    if (!options) options = {};
    if (!credentials) credentials = {};
    options['format'] = 'binary';

    this.client_ = new grpcWeb.GrpcWebClientBase(options);
    this.hostname_ = hostname.replace(/\/+$/, '');
    this.credentials_ = credentials;
    this.options_ = options;
  }

  methodDescriptorSigninTwitch = new grpcWeb.MethodDescriptor(
    '/grpc.Auth/SigninTwitch',
    grpcWeb.MethodType.UNARY,
    github_com_elojah_game_03_pkg_pbtypes_string_pb.String,
    github_com_elojah_game_03_pkg_user_dto_user_pb.SigninResp,
    (request: github_com_elojah_game_03_pkg_pbtypes_string_pb.String) => {
      return request.serializeBinary();
    },
    github_com_elojah_game_03_pkg_user_dto_user_pb.SigninResp.deserializeBinary
  );

  signinTwitch(
    request: github_com_elojah_game_03_pkg_pbtypes_string_pb.String,
    metadata: grpcWeb.Metadata | null): Promise<github_com_elojah_game_03_pkg_user_dto_user_pb.SigninResp>;

  signinTwitch(
    request: github_com_elojah_game_03_pkg_pbtypes_string_pb.String,
    metadata: grpcWeb.Metadata | null,
    callback: (err: grpcWeb.RpcError,
               response: github_com_elojah_game_03_pkg_user_dto_user_pb.SigninResp) => void): grpcWeb.ClientReadableStream<github_com_elojah_game_03_pkg_user_dto_user_pb.SigninResp>;

  signinTwitch(
    request: github_com_elojah_game_03_pkg_pbtypes_string_pb.String,
    metadata: grpcWeb.Metadata | null,
    callback?: (err: grpcWeb.RpcError,
               response: github_com_elojah_game_03_pkg_user_dto_user_pb.SigninResp) => void) {
    if (callback !== undefined) {
      return this.client_.rpcCall(
        this.hostname_ +
          '/grpc.Auth/SigninTwitch',
        request,
        metadata || {},
        this.methodDescriptorSigninTwitch,
        callback);
    }
    return this.client_.unaryCall(
    this.hostname_ +
      '/grpc.Auth/SigninTwitch',
    request,
    metadata || {},
    this.methodDescriptorSigninTwitch);
  }

  methodDescriptorSigninGoogle = new grpcWeb.MethodDescriptor(
    '/grpc.Auth/SigninGoogle',
    grpcWeb.MethodType.UNARY,
    github_com_elojah_game_03_pkg_pbtypes_string_pb.String,
    github_com_elojah_game_03_pkg_user_dto_user_pb.SigninResp,
    (request: github_com_elojah_game_03_pkg_pbtypes_string_pb.String) => {
      return request.serializeBinary();
    },
    github_com_elojah_game_03_pkg_user_dto_user_pb.SigninResp.deserializeBinary
  );

  signinGoogle(
    request: github_com_elojah_game_03_pkg_pbtypes_string_pb.String,
    metadata: grpcWeb.Metadata | null): Promise<github_com_elojah_game_03_pkg_user_dto_user_pb.SigninResp>;

  signinGoogle(
    request: github_com_elojah_game_03_pkg_pbtypes_string_pb.String,
    metadata: grpcWeb.Metadata | null,
    callback: (err: grpcWeb.RpcError,
               response: github_com_elojah_game_03_pkg_user_dto_user_pb.SigninResp) => void): grpcWeb.ClientReadableStream<github_com_elojah_game_03_pkg_user_dto_user_pb.SigninResp>;

  signinGoogle(
    request: github_com_elojah_game_03_pkg_pbtypes_string_pb.String,
    metadata: grpcWeb.Metadata | null,
    callback?: (err: grpcWeb.RpcError,
               response: github_com_elojah_game_03_pkg_user_dto_user_pb.SigninResp) => void) {
    if (callback !== undefined) {
      return this.client_.rpcCall(
        this.hostname_ +
          '/grpc.Auth/SigninGoogle',
        request,
        metadata || {},
        this.methodDescriptorSigninGoogle,
        callback);
    }
    return this.client_.unaryCall(
    this.hostname_ +
      '/grpc.Auth/SigninGoogle',
    request,
    metadata || {},
    this.methodDescriptorSigninGoogle);
  }

  methodDescriptorRefreshToken = new grpcWeb.MethodDescriptor(
    '/grpc.Auth/RefreshToken',
    grpcWeb.MethodType.UNARY,
    github_com_elojah_game_03_pkg_pbtypes_string_pb.String,
    github_com_elojah_game_03_pkg_user_dto_user_pb.SigninResp,
    (request: github_com_elojah_game_03_pkg_pbtypes_string_pb.String) => {
      return request.serializeBinary();
    },
    github_com_elojah_game_03_pkg_user_dto_user_pb.SigninResp.deserializeBinary
  );

  refreshToken(
    request: github_com_elojah_game_03_pkg_pbtypes_string_pb.String,
    metadata: grpcWeb.Metadata | null): Promise<github_com_elojah_game_03_pkg_user_dto_user_pb.SigninResp>;

  refreshToken(
    request: github_com_elojah_game_03_pkg_pbtypes_string_pb.String,
    metadata: grpcWeb.Metadata | null,
    callback: (err: grpcWeb.RpcError,
               response: github_com_elojah_game_03_pkg_user_dto_user_pb.SigninResp) => void): grpcWeb.ClientReadableStream<github_com_elojah_game_03_pkg_user_dto_user_pb.SigninResp>;

  refreshToken(
    request: github_com_elojah_game_03_pkg_pbtypes_string_pb.String,
    metadata: grpcWeb.Metadata | null,
    callback?: (err: grpcWeb.RpcError,
               response: github_com_elojah_game_03_pkg_user_dto_user_pb.SigninResp) => void) {
    if (callback !== undefined) {
      return this.client_.rpcCall(
        this.hostname_ +
          '/grpc.Auth/RefreshToken',
        request,
        metadata || {},
        this.methodDescriptorRefreshToken,
        callback);
    }
    return this.client_.unaryCall(
    this.hostname_ +
      '/grpc.Auth/RefreshToken',
    request,
    metadata || {},
    this.methodDescriptorRefreshToken);
  }

  methodDescriptorPing = new grpcWeb.MethodDescriptor(
    '/grpc.Auth/Ping',
    grpcWeb.MethodType.UNARY,
    github_com_elojah_game_03_pkg_pbtypes_empty_pb.Empty,
    github_com_elojah_game_03_pkg_pbtypes_empty_pb.Empty,
    (request: github_com_elojah_game_03_pkg_pbtypes_empty_pb.Empty) => {
      return request.serializeBinary();
    },
    github_com_elojah_game_03_pkg_pbtypes_empty_pb.Empty.deserializeBinary
  );

  ping(
    request: github_com_elojah_game_03_pkg_pbtypes_empty_pb.Empty,
    metadata: grpcWeb.Metadata | null): Promise<github_com_elojah_game_03_pkg_pbtypes_empty_pb.Empty>;

  ping(
    request: github_com_elojah_game_03_pkg_pbtypes_empty_pb.Empty,
    metadata: grpcWeb.Metadata | null,
    callback: (err: grpcWeb.RpcError,
               response: github_com_elojah_game_03_pkg_pbtypes_empty_pb.Empty) => void): grpcWeb.ClientReadableStream<github_com_elojah_game_03_pkg_pbtypes_empty_pb.Empty>;

  ping(
    request: github_com_elojah_game_03_pkg_pbtypes_empty_pb.Empty,
    metadata: grpcWeb.Metadata | null,
    callback?: (err: grpcWeb.RpcError,
               response: github_com_elojah_game_03_pkg_pbtypes_empty_pb.Empty) => void) {
    if (callback !== undefined) {
      return this.client_.rpcCall(
        this.hostname_ +
          '/grpc.Auth/Ping',
        request,
        metadata || {},
        this.methodDescriptorPing,
        callback);
    }
    return this.client_.unaryCall(
    this.hostname_ +
      '/grpc.Auth/Ping',
    request,
    metadata || {},
    this.methodDescriptorPing);
  }

}

