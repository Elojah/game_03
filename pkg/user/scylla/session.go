...
func (s Store) InsertSession(ctx context.Context, ses user.Session) error {
	q := s.Session.Query(
		`INSERT INTO main.session (id, user_id, token) VALUES (?, ?, ?)`,
		ses.ID,
		ses.UserID,
		ses.Token,
	).WithContext(ctx)
...
func (s Store) FetchSession(ctx context.Context, f user.FilterSession) (user.Session, error) {
	b := strings.Builder{}
	b.WriteString(`SELECT id, user_id, token FROM main.session `)
	...
	if err := q.Scan(&ses.ID, &ses.UserID, &ses.Token); err != nil {
